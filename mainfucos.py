...
class CELoss(torch.nn.Module):
    ...
    def __init__(self, label_smooth=0.1, class_num=60):
        super().__init__()
...
class Processor():
    ...
    def load_model(self):
        ...
        print(self.model)
        #FocalLoss
        #self.loss=CELoss().cuda(output_device)
      #  self.loss = BCEFocalLoss().cuda(output_device)
    ...
    def split_time(self):
        ...
        self.record_time()
        return split_time
  #  def smooth(self,label, classes=60, eta=0.1):
    ...
    def train(self, epoch, save_model=False):
      ...
      #  self.adjust_learning_rate(epoch)
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            # forward
           # print('oo0',label.shape)
           # label=self.sm(label)
            output1,output2 = self.model(data)
            #print('3434',label.shape,output.shape)
            loss = self.loss(output1, label)*0.5+self.loss(output2,label)*0.5
            # backward
            ...
            output=output1*0.5+output2*0.5
            value, predict_label = torch.max(output.data, 1)
           # print('232',label.data.shape,predict_label.shape)
